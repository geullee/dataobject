...
public class GetObs extends Command {

	ArrayList<DataClassInfoHolder> obsSubInfoHolders;
	ArrayList<String> criteriaVariableNames, sourceDataClassVariableNames, 
	targetObservationAddress, targetDataClassVariableNames, targetDataClassVariableTypes,
	obsSubInfoHolderTags;
	...
	String targetObservationName;
	
	public GetObs(ArrayList<String> arguments) {
		super(arguments);
	}
	...
	protected void runCommand() throws Exception {
		...
		targetObservationAddress = DataObject.getPartitionedTokens(arguments.get(0));
		targetObservationName = targetObservationAddress.get(1);
		targetDataClassArray = DataClass.getDataClassArray(targetObservationAddress.get(0));
		...
		obsSubInfoHolders = new ArrayList<DataClassInfoHolder>();
		obsSubInfoHolderTags = new ArrayList<String>();
		
		checkTargetListAppropriateness();
		checkCriteriaVariables();
		
		System.out.print("Retrieving observation '" + targetObservationName +
		...
		reduceInfoHolder();
		constructSubInfoHolders(newInfoHolder, sourceDataClassArray, targetObservationName);
		
		if (obsSubInfoHolders.size() > 0) {

			for (int i = 0; i < obsSubInfoHolders.size(); i++) {
				targetDataClassArray.addSubInfoHolder
				(obsSubInfoHolders.get(i), obsSubInfoHolderTags.get(i));
			}
	...
	InvocationTargetException, NoSuchMethodException, SecurityException {
		...
		for (int i = 0; i < criteriaVariableNames.size(); i++) {
			...
			if (targetCriteriaVariableType.equals(sourceCriteriaVariableType) == false) {
				Console.printErrorMessage("Type of criterion variable '" + criteriaVariableNames.get(i) + 
	...
	InvocationTargetException, NoSuchMethodException, SecurityException { 
		...
		if (doesExist = false) 
			Console.printErrorMessage("Unable to find the target variable '" + targetObservationName + 
		...
		for (int i = 0; i < variableTypes.size(); i++) {
			...
			if (variableType.equals("obs") || variableType.equals("oblist")) {
				...
				DataClassArray nextDataClassArray =
						...
						(dataClassArray.getSubInfoHolder(variableNames.get(i)).getClassName());
				String obListSubInfoHolderTag = 
						address + "/" + variableNames.get(i);

				obsSubInfoHolders.add(obListSubInfoHolder);
				obsSubInfoHolderTags.add(obListSubInfoHolderTag);
	...
	BadLocationException, NoSuchFieldException {
		...
		for (int i = 0; i < targetDataClassArray.size(); i++) {
			...
			do {
				...
				if (isEquivalent(startPointObservation, targetObservation) == true) {

					for (int j = startPoints.get(startPointIndex); 
							j < startPoints.get(startPointIndex + 1); j++) {

						targetObservation.setVariable(targetDataClassInfoHolder, targetObservationName,
								getReducedObservation(sourceDataClassArray.get(j), 
	...
	BadLocationException {
		...
		for (int i = 0; i < targetDataClassArray.size(); i++) {
			...
			do {
				...
				if (isEquivalent(sourceObservation, targetObservation) == true) {

					targetObservation.setVariable(targetDataClassInfoHolder, targetObservationName,
							getReducedObservation(sourceObservation, 
							sourceDataClassInfoHolder, newInfoHolder));
					observationsIncluded++;
		...
		for (int i = 0; i < sourceDataClassArray.size(); i++) {
			...
			do {
				...
				if (isEquivalent(sourceObservation, targetObservation) == true) {
					...
					targetObservation.setVariable(targetDataClassInfoHolder, targetObservationName,
							getReducedObservation(sourceDataClassArray.get(i), 
							sourceDataClassInfoHolder, newInfoHolder));
					observationsIncluded++;
					isDone = true;
	...
	private boolean isLarger(Observation targetObservation,
		...
		String startPointFirstCriterionValue = 
				startPointObservation.getStringTypeVariable(sourceDataClassInfoHolder, firstCriterionName);
		String variableType = 
	...
	private void reduceInfoHolder() {
		...
		targetDataClassArray.addSubInfoHolder(newInfoHolder, targetObservationName);
